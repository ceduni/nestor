...
export default function Calendar({ space }: { space: Space }) {
  ...
  const handleBackArrowClick = () => setCalendarDate((prev) => sub(prev, { weeks: 1 }));
  const handleForwardArrowClick = () => setCalendarDate((prev) => add(prev, { weeks: 1 }));
  ...
  const calculateEventHeight = (startTime: Date, endTime: Date, height: number) => {
        ...
        <div className="calendar-header-container">
          ...
          <div className="calendar-header-left-container">
            ...
            <div className="calendar-today-button-container">
              <IoIosArrowBack className="arrow" onClick={handleBackArrowClick} />
              <div onClick={handleTodayClick}>Aujourd'hui</div>
              <IoIosArrowForward className="arrow" onClick={handleForwardArrowClick} />
            </div>
          </div>
        </div>
        <div>
          ...
          <div className="calendar-columns-container">
            ...
            <div className="calendar-day-cells-container">
              {Array.from({ length: 175 }).map((_, index) => {
                ...
                if (isFirstRow) {
                  return <div key={index} className="calendar-day-cells-item-first-row"></div>;
                }
