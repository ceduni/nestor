...
export default function Calendar({spaceDetail}) {
    ...
    const displayDate = (start, end) => {
        ...
        setDateRange(format(start, "MMMM dd",{locale})
            .concat(" - ")
    ...
    const handleResize = (e) => {
        if(typeof e.extendedProps.isBooked !== undefined && !e.extendedProps.isBooked) {
            e.setProp("borderColor", '#06940a')
            e.setProp('textColor', '#FFF')
            e.setProp('backgroundColor', '#06940a')
        }
       else if(typeof e.extendedProps.isBooked !== undefined && e.extendedProps.isBooked) {
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            {showModal && <Modal event={event} setShowModal={setShowModal}/>}
        </div>
    )
}
